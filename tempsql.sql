-- =====================================================
-- CARECONNECT PLATFORM - COMPLETE DATABASE SETUP (CORRECTED)
-- Last updated: 2025-09-27 (rewritten with fixes)
-- =====================================================

-- Ensure required extensions (use pgcrypto for gen_random_uuid)
CREATE EXTENSION IF NOT EXISTS "pgcrypto";
CREATE EXTENSION IF NOT EXISTS "uuid-ossp"; -- kept in case other code expects it

-- =====================================================
-- 0. Defensive: Ensure updated_at exists where triggers rely on it
-- =====================================================
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'job_history') THEN
        ALTER TABLE job_history ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'aborted_jobs') THEN
        ALTER TABLE aborted_jobs ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'job_location_updates') THEN
        ALTER TABLE job_location_updates ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();
    END IF;
END $$;
...
-- Profiles: add commonly used fields if missing
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='profiles') THEN
...
-- Add constraints for rating columns (guarded by existence checks)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='profiles' AND column_name='average_rating') THEN
        IF NOT EXISTS (
            SELECT 1 FROM pg_constraint c
            JOIN pg_class t ON c.conrelid = t.oid
            WHERE c.contype = 'c' AND t.relname = 'profiles' AND c.conname = 'profiles_average_rating_check'
        ) THEN
            ALTER TABLE profiles ADD CONSTRAINT profiles_average_rating_check CHECK (average_rating >= 0 AND average_rating <= 5);
        END IF;
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='profiles' AND column_name='total_ratings') THEN
        IF NOT EXISTS (
            SELECT 1 FROM pg_constraint c
            JOIN pg_class t ON c.conrelid = t.oid
            WHERE c.contype = 'c' AND t.relname = 'profiles' AND c.conname = 'profiles_total_ratings_check'
        ) THEN
            ALTER TABLE profiles ADD CONSTRAINT profiles_total_ratings_check CHECK (total_ratings >= 0);
        END IF;
    END IF;
EXCEPTION
    WHEN OTHERS THEN RAISE WARNING 'Skipping rating constraints addition: %', SQLERRM;
END $$;

-- Jobs: add missing columns if they don't exist
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='jobs') THEN
...
-- =====================================================
-- 2. UPDATE/CREATE CONSTRAINTS (SAFE)
-- =====================================================

-- Update jobs status enum/check to include new statuses (including 'aborted')
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='jobs') THEN
        -- Remove existing check if present
    ...
    END IF;
EXCEPTION
    WHEN OTHERS THEN RAISE WARNING 'Could not add jobs_status_check: %', SQLERRM;
END $$;
...
BEGIN
    ...
    END IF;
EXCEPTION
    WHEN OTHERS THEN RAISE WARNING 'Could not add jobs_urgency_check: %', SQLERRM;
END $$;
...
-- Drop and recreate job_history to ensure consistent schema (safe because we drop only if exists)
...
CREATE TABLE IF NOT EXISTS aborted_jobs (
    ...
    CHECK (aborted_by = elderly_id OR aborted_by = helper_id) -- Only participants can abort
);
...
-- Generic updated_at function (sets NEW.updated_at = now())
...
-- Add triggers (only if the target tables exist)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='profiles') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='aborted_jobs') THEN
        DROP TRIGGER IF EXISTS update_aborted_jobs_updated_at ON aborted_jobs;
        CREATE TRIGGER update_aborted_jobs_updated_at
            BEFORE UPDATE ON aborted_jobs
            FOR EACH ROW
            EXECUTE FUNCTION update_updated_at_column();
    END IF;
END $$;
...
-- Move job to history function (handles completed and fully_completed)
CREATE OR REPLACE FUNCTION move_job_to_history()
RETURNS TRIGGER AS $$
BEGIN
    -- When transitioning into a completed-like state, insert a job_history row (if not exists)
       ...
       (OLD.status IS NULL OR OLD.status NOT IN ('completed', 'fully_completed')) THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM job_history WHERE job_id = NEW.id) THEN
            INSERT INTO job_history (
                ...
                job_created_at, job_started_at, job_ended_at, helper_arrived_at, archived_at
            )
            SELECT
                NEW.id,
                NEW.elderly_id,
                NEW.helper_id,
                COALESCE(ep.full_name, ep.email) AS elderly_name,
                COALESCE(hp.full_name, hp.email) AS helper_name,
                ...
                NEW.job_started_at,
                NEW.job_ended_at,
                NEW.helper_arrived_at,
                NOW()
            FROM profiles ep
            LEFT JOIN profiles hp ON hp.id = NEW.helper_id
            WHERE ep.id = NEW.elderly_id;
        END IF;
    END IF;
    RETURN NEW;
EXCEPTION
...
-- Create trigger for job history (only if jobs table exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='jobs') THEN
...
BEGIN
    ...
    INSERT INTO job_location_updates (job_id, helper_id, location, recorded_at, updated_at)
    VALUES (p_job_id, p_helper_id, p_location, NOW(), NOW());
END;
$$ LANGUAGE plpgsql;

-- Function to abort a job (Supabase auth.uid() used)
...
BEGIN
    -- Get job details and ensure caller is a participant and job not already final
...
-- =====================================================
-- 6. RLS POLICIES (Supabase-specific)
-- =====================================================
...
-- =====================================================
-- 7. CREATE STORAGE BUCKET AND POLICIES (Supabase-specific)
-- =====================================================
...
-- Note: these rely on Supabase storage helper functions (storage.foldername, storage.extension)
CREATE POLICY IF NOT EXISTS "Public access to profile photos"
...
CREATE POLICY IF NOT EXISTS "Users can upload their own Aadhaar documents"
    ON storage.objects FOR INSERT
    TO authenticated
    WITH CHECK (
...
CREATE POLICY IF NOT EXISTS "Users can view their own Aadhaar documents"
    ON storage.objects FOR SELECT
    TO authenticated
    USING (
        bucket_id = 'aadhaar-documents' AND 
...
CREATE POLICY IF NOT EXISTS "Users can update their own Aadhaar documents"
    ON storage.objects FOR UPDATE
    TO authenticated
    USING (
        bucket_id = 'aadhaar-documents' AND 
...
CREATE POLICY IF NOT EXISTS "Users can delete their own Aadhaar documents"
    ON storage.objects FOR DELETE
    TO authenticated
    USING (
        bucket_id = 'aadhaar-documents' AND 
...
-- =====================================================
/*
-- Sample admin user (create this user in Supabase Auth first)
...
-- =====================================================
-- 11. DATA CLEANUP AND FIXES
-- =====================================================

-- Remove duplicate job_history rows if any
DELETE FROM job_history a USING job_history b 
WHERE a.id > b.id AND a.job_id = b.job_id;

-- Ensure jobs referenced by aborted_jobs have status 'aborted'
UPDATE jobs 
SET status = 'aborted', updated_at = NOW()
WHERE id IN (
    SELECT job_id 
...
-- Sync profile ratings from ratings table
UPDATE profiles 
SET 
...
-- Verify core tables exist (adjust counts if you add/remove tables)
SELECT 
...
SELECT 
    'New Tables: ' || count(*) || '/3' as status
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('aborted_jobs', 'ratings', 'job_location_updates');

-- Verify storage buckets exist (Supabase storage schema)
SELECT 
...
-- =====================================================
-- FINAL SUCCESS MESSAGE (SELECT)
-- =====================================================

SELECT '
🎉 CARECONNECT DATABASE SETUP (CORRECTED) COMPLETED! 🎉

Notes:
- UUIDs use gen_random_uuid(); pgcrypto extension is enabled.
- updated_at columns ensured where needed.
- aborted_jobs check constraint fixed to use OR.
- move_job_to_history() insert logic improved to handle NULL helper_id.
- Script assumes Supabase environment (auth.uid(), storage helpers). If you need plain Postgres version, request it.
